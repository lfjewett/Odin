...
Schwab Broker API (REST \+ WebSocket)
  ...
  \- Manages candle lifecycle: LIVE \-\> CLOSED \-\> RECONCILED \-\> FINAL
...
| LIVE | Candle is forming. Updated every tick. Not yet closed. |
| :---- | :---- |
| **CLOSED** | Broker streaming WebSocket says the minute closed. May differ from REST history. |
| **RECONCILED** | REST history API has returned a value for this timestamp. Replaces CLOSED. |
...
candles table:
  ...
  state        ENUM(LIVE, CLOSED, RECONCILED, FINAL)
...
| **Candle corrections** | Schwab reconciles candle data post-close and EOD. Price Agent tracks state (LIVE/CLOSED/RECONCILED/FINAL) and broadcasts typed CANDLE\_CORRECTION events. |
