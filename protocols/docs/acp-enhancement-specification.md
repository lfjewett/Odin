...
## Ratified Decision Notes (2026-03-07)

This enhancement document is now superseded by `spec/ACP.md` and the schemas for normative behavior.

Ratified protocol decisions:
- Candle lifecycle states are `partial`, `provisional_close`, `session_reconciled`, `final`
- `GET /history` returns latest-snapshot candles (one record per candle id) including current `bar_state` and `rev`
- Backend always rebroadcasts REST-detected changes as `candle_correction`
- Transition into `session_reconciled` can occur even when OHLCV values are unchanged
- `final` timing is backend-policy driven (protocol does not fix a strict clock)
- No ACP-level trade execution gating requirements
...
- `provisional_close`: Candle closed from the streaming close signal
- `session_reconciled`: Backend has entered reconciliation stage using REST confirmation/polling
- `final`: Backend terminal state per finalization policy
...
type BarState = 
  ...
  | "provisional_close" // Candle closed (streaming API close)
  | "session_reconciled" // REST reconciliation stage (may or may not change values)
  | "final"        // Terminal state, timing is backend policy
```

#### State Transitions
```
partial → partial (many times, intrabar updates)
partial → provisional_close (on minute close)
provisional_close → session_reconciled (first REST confirmation/poll, even if unchanged)
session_reconciled → session_reconciled (zero or more reconciliation updates)
session_reconciled → final (when backend policy finalizes the candle)
```

#### Revision Semantics
- `rev` increments each time a candle with the same `id` is updated
- `partial` updates: rev increments freely
- `provisional_close`: rev increments on close boundary
- `session_reconciled`: rev may increment even when values are unchanged
- `final`: terminal state, `rev` frozen
...
14:33:00 → {id: "SPY:1m:1709827920", rev: 3, bar_state: "provisional_close", close: 512.44}

[10 seconds later, REST API reconciliation runs]
14:33:10 → {id: "SPY:1m:1709827920", rev: 4, bar_state: "session_reconciled", close: 512.43}
```
...
- [ ] Update `schemas/ohlc.json`:
  - [ ] Add `bar_state` enum: `["partial", "provisional_close", "session_reconciled", "final"]`
...
```json
{
  ...
  "session_id": "abcd-1234",
  "seq": 10020,
  "candle": { /* provisional_close candle */ }
}
```
